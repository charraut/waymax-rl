...
def make_model(
    ...
    Returns:
        a model
    """
    warnings.warn("make_model is deprecated, use make_{policy|q|value}_network instead.")
    dummy_obs = jnp.zeros((1, obs_size))

    module = MLP(layer_sizes=layer_sizes, activation=activation)
    model = FeedForwardNetwork(init=lambda rng: module.init(rng, dummy_obs), apply=module.apply)
...
def make_models(policy_params_size: int, obs_size: int) -> Tuple[FeedForwardNetwork, FeedForwardNetwork]:
    ...
    """
    warnings.warn("make_models is deprecated, use make_{policy|q|value}_network instead.")
